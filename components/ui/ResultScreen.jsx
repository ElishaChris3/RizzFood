...
function getDiscount(score) {
  if (score >= 300)
    return { pct: "5%", label: "LEGENDARY!", emoji: "👑", color: "#B14EFF" };
  if (score >= 200)
    return { pct: "4%", label: "AMAZING!", emoji: "🔥", color: "#FF2D87" };
  if (score >= 100)
    return { pct: "3%", label: "GREAT!", emoji: "⚡", color: "#FFB800" };
  if (score >= 50)
    return { pct: "2%", label: "GOOD!", emoji: "🍊", color: "#FF6B2B" };
  return { pct: "1%", label: "NICE TRY!", emoji: "😅", color: "#7FFF00" };
}
...
export default function ResultScreen({ score, onPlayAgain, onHome }) {
  ...
  return (
    <div className="relative w-full min-h-screen flex flex-col items-center justify-center overflow-hidden bg-[#0D0D0D] px-4">
      ...
      <div
        className="blob w-56 h-56 bg-neon-purple top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2"
        style={{ opacity: 0.1 }}
      />
      ...
      >
        ...
        <motion.div
          ...
          style={{
            background: `${discount.color}22`,
            border: `2px solid ${discount.color}60`,
        ...
        <motion.div
          ...
          style={{
            background:
              "linear-gradient(135deg, rgba(255,107,43,0.12), rgba(255,184,0,0.08))",
        ...
        >
          ...
          <div
            ...
            style={{
              background: `radial-gradient(circle, ${discount.color}, transparent)`,
            }}
          />
        </motion.div>
